...
namespace RedSys.Common.Workflow
{
    ...
    public static class DateTimeExtensions
    {
        ...
        public static DateTime AddBusinessDays(this DateTime current, int days, SPList spList)
        {
            var sign = Math.Sign(days);
            var unsignedDays = Math.Abs(days);
            ...
            if (spList != null)
            {
                ...
                spQuery.Query =
                    string.Format("<Where><Geq><FieldRef Name=\"VacationDay\" /><Value Type=\"DateTime\">{0}</Value></Geq></Where>", SPUtility.CreateISO8601DateTimeFromSystemDateTime(DateTime.Now));

                SPListItemCollection listItemCollection = null;
                listItemCollection = spList.GetItems(spQuery);

                if (listItemCollection != null && listItemCollection.Count != 0)
                {
                    listData.AddRange(from SPListItem listItem in listItemCollection
                                      select listItem.GetFieldValueDateTime("VacationDay").Value);
                }
            }
            else
            {
            ...
            for (var i = 0; i < unsignedDays; i++)
            {
                var match = listData.Where(s => s.Date == current.Date);
                do
                {
                    current = current.AddDays(sign);
                    match = listData.Where(s => s.Date == current.Date);
                }
                while (match.Count() != 0);
            }
            return current;
