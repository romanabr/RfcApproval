...
var SPUtility = (function ($) {
	...
	*/
	 ...
	 * Supports single select lookup fields
	 */
	function SPPSELookupField(fieldParams, elemInputs) {
		SPField.call(this, fieldParams);
		if (this.Controls === null) {
			return;
		}
		...
		if (1 === elemInputs.length) {
			// autocomplete lookup
			this.LookupSelection = $(elemInputs[0]);
		} else {
	...
	// Inherit from SPField
	SPPSELookupField.prototype = Object.create(SPField.prototype);

	SPPSELookupField.prototype.GetValue = function () {
		return decodeURIComponent(this.LookupSelection.find('input').val());
	};

	SPPSELookupField.prototype.GetValueID = function () {
		return this.LookupSelection.find('div[item-id]').attr('item-id');
	};

	SPPSELookupField.prototype.SetValue = function (value) {
		this.LookupSelection.find('input').val(value);
		return this;
	};
	...
	function getSPFieldFromType(spFieldParams) {
		...
		switch (spFieldParams.type) {
			...
			case 'SPFieldURL':
				...
				break;
			case 'SPFieldPSELookup':
				controls = $(spFieldParams.controlsCell).find('.lookupSelection');
				if (controls.length > 0) {
					field = new SPPSELookupField(spFieldParams, controls);
				}
				break;
			case 'SPFieldLookup':
				// is this a normal dropdown field?
